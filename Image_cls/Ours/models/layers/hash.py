...
def custom_hash(passport_img=None, hash_length=512):
    print("Generating passport signature with SHA-512 hash...")

    if hash_length > 512:
        return "Hash length must be less than or equal to 512 bits"
    if passport_img is None:
        image_path = './data/trigger_set/pics/1.jpg'
        print("Load image from {} for signature generation.".format(image_path))
        # Open the image
