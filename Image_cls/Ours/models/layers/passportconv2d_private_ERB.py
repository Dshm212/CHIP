...
class PassportPrivateBlockERB(nn.Module):
    def __init__(self, i, o, ks=3, s=1, pd=1, passport_kwargs={}):
        ...
        self.ERB_fc = nn.Sequential(
            nn.Linear(o, o // 4, bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Linear(o // 4, o, bias=True)
        )
    ...
    def reset_b(self, x):
        print("Reset b")
        b = custom_hash(self.skey_private,
                        hash_length=self.o)  # NOTE: bit information to store by using Alice signitures
    ...
    def get_scale(self, force_passport=False, ind=0):
        if self.scale is not None and not force_passport and ind == 0:
            ...
            _scale = scale.view(1, c)
            scale = self.ERB_fc(_scale).view(1, c, 1, 1) + scale
    ...
    def get_bias(self, force_passport=False, ind=0):
        if self.bias is not None and not force_passport and ind == 0:
            ...
            _bias = bias.view(1, c)
            bias = self.ERB_fc(_bias).view(1, c, 1, 1) + bias
