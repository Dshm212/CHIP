...
def pruning_resnet(model, pruning_perc):
    ...
    #         if 'fc' not in name:
    #             prune.l1_unstructured(module, name='weight', amount=pruning_perc / 100)
...
def pruning_resnet2(model, pruning_perc, type_prune='rd'):
    ...
    #     if 'fc' not in name:
    #         num_weights = p.numel()
    #         num_prune = int(pruning_perc * num_weights / 100)
    #
    #         mask = torch.ones(num_weights, dtype=torch.float32, device=p.device)
    #         prune_indices = np.random.choice(num_weights, num_prune, replace=False)
...
# arch = 'resnet'
# passport_config = 'passport_configs/resnet18_passport_l2.json'
# dataset  = 'cifar100'
#
# batch_size = 64
