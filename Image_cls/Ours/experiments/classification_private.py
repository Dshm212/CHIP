...
class ClassificationPrivateExperiment(Experiment):
    ...
    def fake_attack(self):
        ...
        basedir = '/mnt/ssd4/chaohui/IP_protection/CHIP/Image_cls/Ours/passport_attack/' + task_name + '/'
        ...
        for m in self.model.modules():
            ...
            if isinstance(m, PassportBlock) or isinstance(m, PassportPrivateBlock):
                ...
                for layer in m.fc1.modules():
                    if isinstance(layer, nn.Linear):
                        nn.init.xavier_uniform_(layer.weight)
                        for p in layer.parameters():
                            p.requires_grad = True

                for layer in m.fc2.modules():
                    if isinstance(layer, nn.Linear):
                        nn.init.xavier_uniform_(layer.weight)
                        for p in layer.parameters():
                            p.requires_grad = True
        ...
        for user_idx in range(self.idx):
            user_model_path = f'{basedir}/{self.type}_{str(user_idx + 1)}/{self.arch}-v3-last-{self.dataset}-{self.type}-{self.flipperc:.1f}-e{epochs}.pth'
            user_passport = torch.load(user_model_path)['fakepassport']
            user_passport_list.append(user_passport)

        # user1_path = "./passport_attack/alexnet_caltech-101_v4_last5/oracle_1/alexnet-v3-last-caltech-101-oracle-0.0-e40.pth"
        # user1_passport = torch.load(user1_path)['fakepassport']

        # user2_path = "./passport_attack/resnet_caltech-101_v4_last1/oracle_2/resnet-v3-last-caltech-101-oracle-0.0-e40.pth"
        # user2_passport = torch.load(user2_path)['fakepassport']
    ...
    def check(self):
        ...
        acc_array = np.zeros((3, 3))
        cs_arracy = np.zeros((3, 3))
        ...
        for i in range(3):
            for j in range(3):
                ...
                owner_path = f'./passport_attack/alexnet_caltech-101_v4_last5/oracle_{str(i + 1)}/alexnet-v3-last-caltech-101-oracle-0.0-e40.pth'
                attack_path = f'./passport_attack/alexnet_caltech-101_v4_last5/oracle_{str(j + 1)}/alexnet-v3-last-caltech-101-oracle-0.0-e40.pth'
