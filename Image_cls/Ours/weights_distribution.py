...
def run_maximize(rep=1, flipperc=0, arch='alexnet', dataset='cifar10', scheme=1,
    ...
    for m in model.modules():
        if isinstance(m, PassportBlock) or isinstance(m, PassportPrivateBlock):
            ...
            # #initial fake key random
            # m.register_parameter(keyname, nn.Parameter( torch.randn(*key.size()) ))
            # m.register_parameter(skeyname, nn.Parameter( torch.randn(*skey.size()) ))
...
if __name__ == '__main__':
    ...
    args.scheme = 2
    args.loadpath = "/data-x/g12/zhangjie/DeepIPR/ours/alexnet_cifar100_v2_all-random-our4/"
    print(args.loadpath)
    args.passport_config = 'passport_configs/alexnet_passport_5.json'
    args.rep = "50%S"
    args.flipperc = 0.5
